...
const includeEpochs = require('./transformers/includeEpochs');
...
const flow = (phrases, { now } = { now: new Date() }) => {
  const groups = _.flow(
    ...
    // To convert uppercase, allowing joining phrases:
    p => p.map(convertSynonyms({ createMap: () => [{ from: ['navy'], to: ['navy'] }] })),
    ...
    p => p.map(convertSynonyms({ createMap: () => [{ from: ['u.s.', 'navy'], to: ['United', 'States', 'Navy'] }] })),
    p => p.map(addCommas),
    p => p.map(groupDates),
